...
const Notification = ({ notification }: { notification: INotification }) => {
  ...
  return (
    <li
      className={cn(
        'group border-border bg-card relative flex flex-col gap-4 rounded-lg border p-4 transition-colors duration-150 sm:flex-row sm:items-start sm:justify-between',
        notification.isRead ? 'opacity-80' : 'bg-accent/40',
      )}
    >
      {!notification.isRead && (
        <span
          className="bg-primary absolute top-6 -left-1 h-2 w-2 rounded-full shadow-[0_0_8px_var(--primary)]"
          aria-hidden
        />
      )}
