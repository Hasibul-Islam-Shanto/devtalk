...
The backend image includes the API only. MongoDB and Redis are started as Compose services, so you do not need them installed locally.
...
To run the frontend together with this API, MongoDB, and Redis, use the workspace-root `docker compose up --build` instead.
